...
public class TodayFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener{
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.fragment_today, null);
        view = v;
        drawerListView = (ListView) v.findViewById(R.id.main_page_listview);
        swipeRefreshLayout = (SwipeRefreshLayout) v.findViewById(R.id.today_swipe_layout);
            ...
            try {
                while (true) {
                    Document doc = Jsoup.connect("https://dnevnik.ru/user/")
                            ...
                            .get();
                    String todayUrl = doc.select(".col23.first")
                            .select(".panel.blue2")
                            .select(".rounded")
                            .select(".cc")
                            .select(".col50.first")
                    ...
                    try {
                        ...
                        Document docToday = Jsoup.connect(todayUrl)
                                .cookies(CookiesContainer.getInstance().getCookies())
                                .get();
